...
public abstract class LoginDialog extends Dialog{
	...
	private final ASsettings mSettings;
	private final EditText loginUsernameInput;
	private final EditText loginPasswordInput;

	
	public LoginDialog(final Activity activity, ASsettings settings, boolean finishActivityIfCanceled){
		super(activity);
		mActivity = activity;
		mSettings = settings;
		...
		String u = settings.getUsername();
		loginUsernameInput.setText(u);
		...
		String p = settings.getPassword();
		loginPasswordInput.setText(p);
	...
	private void handleLoginChosen() {
		...
		ASsettings.setUsername(user);
		ASsettings.setPassword(password);
